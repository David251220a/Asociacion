...
class PlanillaAporteService
{
    public function generarDetalle(int $mes, int $anio, int $tipoAsociadoId)
    {
        ...
        $coleccion = collect($rows)->map(function ($item) use ($periodo) {
    ...
    public function guardarPlanilla(int $mes, int $anio, int $tipoAsociadoId)
    {
        return DB::transaction(function () use ($mes, $anio, $tipoAsociadoId) {
            ...
            $tipoGeneracion = $tipoAsociadoId == 3 ? 'APORTE' : 'JUBILADO';
            $loteGeneracion = "{$tipoGeneracion}_" . str_pad($mes, 2, '0', STR_PAD_LEFT) . "_{$anio}_{$tipoAsociadoId}_" . strtoupper(Str::random(6));
            ...
            // PASIVO: una sola cabecera con institucion_id = 1
            if ($tipoAsociadoId != 3) {
            ...
            // APORTANTE: una planilla por institución
            ...
            foreach ($gruposPorInstitucion as $institucionId => $items) {
                ...
                if ($existe) {
                    throw new \Exception("Ya existe una planilla generada para esa institución en ese mes y año.");
                }
    ...
    private function guardarDetallesAporte(Planilla $planilla,$items,Carbon $ahora,int $usuarioId): void {
        /*
        |--------------------------------------------------------------------------
        | INSERTAR PLANILLA DETALLE
        |--------------------------------------------------------------------------
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | RECUPERAR LOS DETALLES INSERTADOS
        |--------------------------------------------------------------------------
        */
        ...
        ->whereIn(
            'asociado_id',
            collect($items)->pluck('asociado_id')
        )
        ->lockForUpdate()
        ->get()
        ->keyBy('asociado_id');
        ...
        */
        ...
        foreach ($items as $item) {
            $detalle = $detallesGuardados->get($item['asociado_id']);
