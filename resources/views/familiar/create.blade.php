    ...
    <div  class="col-lg-12 layout-spacing">
        <div class="statbox widget box box-shadow">
            <div class="widget-content widget-content-area">
                ...
                <form id="form_general" action="{{route('familiar.store', $persona)}}" method="post" enctype="multipart/form-data" 
                        onsubmit="
                                ...
                                <div class="form-group col-md-3">
                                    ...
                                    <select name="tipo_familiar" id="tipo_familiar" class="form-control">
                                        @foreach ($tipo as $item)
                                            <option {{ (old('tipo_familiar') == $item->id ? 'selected' : '' ) }} value="{{$item->id}}">{{$item->descripcion}}</option>
                                        @endforeach
                                    </select>
                                </div>
